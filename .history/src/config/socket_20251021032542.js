...
// In src/config/socket.js - add these event handlers

socket.on('join_seller_room', (data) => {
...
// Emit events from controllers when order status changes
const emitOrderStatusUpdate = (order) => {
  if (io) {
    ...
    if (order.driver) {
      io.to(`driver_${order.driver}`).emit('order_status_update', order);
    }
...
socket.on('join_seller_approval', (data) => {
  if (socket.userRole === 'seller') {
    socket.join(`seller_approval_${socket.userId}`);
  }
});

// Emit function for seller approval
