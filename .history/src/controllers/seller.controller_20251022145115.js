  ...
  try {
    ...
    for (let i = 0; i < 3; i++) {
      const location = await Location.create({
        ...
        location: {
          type: 'Point',
          coordinates: [
            67.0 + (Math.random() * 2), // Karachi to Islamabad longitude range
            24.0 + (Math.random() * 5)  // Karachi to Islamabad latitude range
          ]
        },
        isActive: true
      });
    ...
    for (const location of locations) {
      const inventory = await Inventory.create({
        ...
        city: location.city,
        pricePerKg: 150 + Math.floor(Math.random() * 50),
        cylinders: {
          '5kg': { quantity: 50 + Math.floor(Math.random() * 100), price: 800 },
          '10kg': { quantity: 30 + Math.floor(Math.random() * 80), price: 1500 },
          '15kg': { quantity: 20 + Math.floor(Math.random() * 50), price: 2200 },
          '45kg': { quantity: 10 + Math.floor(Math.random() * 30), price: 6500 }
        },
        addOns: [
          { name: 'Regulator', price: 300 },
          { name: 'Hose Pipe', price: 150 },
          { name: 'Pressure Gauge', price: 200 }
        ]
      });
    ...
    // Create cylinders
    const cylinders = [];
    const cylinderSizes = ['5kg', '10kg', '15kg', '45kg'];
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const cylinder = orderType === 'return' ? cylinders.filter(c => c.buyer && c.buyer.toString() === buyer._id.toString())[0] : null;
      ...
      const order = await Order.create({
        ...
        payment: {
          ...
          status: ['completed', 'in_transit', 'completed'].includes(status) ? 'completed' : 'pending',
...
// Helper function to create test buyers
const createTestBuyers = async () => {
  ...
  const buyerData = [
    ...
    {
      fullName: 'Fatima Ali',
      phoneNumber: `0312${Math.floor(1000000 + Math.random() * 9000000)}`,
      email: 'fatima.ali@example.com',
      userType: 'domestic',
      cnic: '23456-7890123-4'
    },
    {
      fullName: 'Bilal Enterprises',
      phoneNumber: `0321${Math.floor(1000000 + Math.random() * 9000000)}`,
      email: 'bilal.enterprises@example.com',
      userType: 'commercial',
      cnic: '34567-8901234-5'
    },
    {
      fullName: 'Sara Restaurant',
      phoneNumber: `0333${Math.floor(1000000 + Math.random() * 9000000)}`,
      email: 'sara.restaurant@example.com',
      userType: 'commercial',
      cnic: '45678-9012345-6'
    },
    {
      fullName: 'Usman Traders',
      phoneNumber: `0345${Math.floor(1000000 + Math.random() * 9000000)}`,
      email: 'usman.traders@example.com',
      userType: 'commercial',
...
// Helper function to get a random driver
