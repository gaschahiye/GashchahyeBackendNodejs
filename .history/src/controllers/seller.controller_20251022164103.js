...
const generateBulkTestData = async (req, res, next) => {
  try {
    ...
    // ✅ FIXED INVENTORIES with correct cylinder keys and addon structure
    const inventories = [];
    for (const location of locations) {
      const inventory = await Inventory.create({
        ...
        addOns: [
          { title: 'Regulator', quantity: 50, price: 300 },
          { title: 'Pipe', quantity: 40, price: 200 },
          { title: 'Safety Cap', quantity: 60, price: 100 }
        ]
      });
    ...
    // Create cylinders
    const cylinders = [];
    const cylinderSizes = ['4.5kg', '6kg', '11.8kg', '15kg']; // ✅ fixed to match schema
    for (let i = 0; i < 50; i++) {
